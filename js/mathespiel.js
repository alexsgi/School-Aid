  ...
  submitBtn.addEventListener("click", () => {
    ...
    if (userInput) {
      ...
      if (userInput == answerValue) {
        stopGame(`Yippie!! <span>Richtige</span> Antwort`);
      }
      //If user inputs operator other than +,-,*
      else if (operatorQuestion && !operators.includes(userInput)) {
        errorMessage.classList.remove("hide");
        errorMessage.innerHTML = "Gebe eine gültigen Operator an";
      }
      //If user guessed wrong answer
      else {
        stopGame(`Opps!! <span>Falsche</span> Antwort`);
      }
    }
    //If user input is empty
    else {
      errorMessage.classList.remove("hide");
      errorMessage.innerHTML = "Feld kann nicht leer sein";
    }
  });
};
//Start Game
